...
@section('page_title', 'Editar salida')

@section('page_subtitle')
  Modifica la salida. Esto revertirá existencias y aplicará los nuevos valores.
@endsection

@section('page_actions')
  <x-btn variant="secondary" href="{{ route('salidas.show', $salida) }}">
...
@section('content')
  <x-card>
    ...
    @error('detalles')
      <div class="mx-6 mt-6 rounded-xl border border-red-200 bg-red-50 text-red-700 px-4 py-3">
        <div class="font-semibold">No se pudo actualizar la salida</div>
        <div class="text-sm mt-1">{{ $message }}</div>
      </div>
    @enderror

    <form method="POST" action="{{ route('salidas.update', $salida) }}" class="p-6 space-y-6">
      @csrf
      @method('PUT')
      ...
      <div class="grid grid-cols-1 sm:grid-cols-12 gap-4">
        <div class="sm:col-span-3">
          ...
          <input
            type="date"
            name="fecha"
            value="{{ old('fecha', \Illuminate\Support\Carbon::parse($salida->fecha)->format('Y-m-d')) }}"
        ...
        <div class="sm:col-span-4">
          ...
          >
            @foreach($almacenes as $a)
              <option value="{{ $a->id }}" @selected(old('almacen_id', $salida->almacen_id) == $a->id)>{{ $a->nombre }}</option>
            @endforeach
          </select>
        ...
        <div class="sm:col-span-3">
          ...
          >
            @foreach($tipos as $t)
              <option value="{{ $t }}" @selected(old('tipo', $salida->tipo) === $t)>{{ strtoupper($t) }}</option>
            @endforeach
          </select>
        ...
        <div class="sm:col-span-12">
          ...
          >{{ old('observaciones', $salida->observaciones) }}</textarea>
      ...
      <div class="border rounded-2xl overflow-hidden">
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full text-sm">
            ...
            @php
              $oldDetalles = old('detalles', $detalles ?? []);
                    ...
                    >
                      ...
                      @foreach($insumos as $i)
                        <option
                          ...
                          @selected((string)($d['insumo_id'] ?? '') === (string)$i->id)
                        >
                          {{ $i->sku }} — {{ $i->nombre }}
                        </option>
                      @endforeach
      ...
      <div class="flex justify-end gap-2">
        ...
        <x-btn type="submit" onclick="return confirm('¿Guardar cambios? Esto ajustará existencias.');">
  ...
  <script>
    ...
    function getRow(el) { return el.closest('tr.detalle-row'); }
    ...
    function recalcTotal() {
      ...
      document.querySelectorAll('#detallesBody tr.detalle-row').forEach(tr => {
        ...
        const costInput = tr.querySelector('input[name*="[costo_unitario]"]').value || '0';
