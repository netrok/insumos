...
@section('content')
  <div class="max-w-4xl">
    <x-card>
      <form method="POST" action="{{ route('almacenes.update', $almacen) }}" class="p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="flex items-center justify-between gap-4 rounded-xl border border-gray-200 p-4 bg-white">
          ...
          <label class="inline-flex items-center cursor-pointer select-none">
            <input type="checkbox" name="activo" value="1" class="sr-only peer" @checked(old('activo', $almacen->activo))>
